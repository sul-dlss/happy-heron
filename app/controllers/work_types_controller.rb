...
class WorkTypesController < ApplicationController
  ...
  def update_work_type(work, work_type, subtypes)
    ...
    new_version = NewVersionService.dup(work.head, increment_version: true, save: true,
                                                   version_description: 'work type changed',
                                                   state: :version_draft) do |work_version|
  ...
  def description_for(original_version, new_version)
    if original_version.work_type != new_version.work_type && original_version.subtype != new_version.subtype
      return 'work type modified, subtypes modified'
    end
