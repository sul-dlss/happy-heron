...
class WorksMailer < ApplicationMailer
  ...
  def new_version_reminder_email
    ...
    subject = "Reminder: Updates to your deposit in the #{@work.collection_name} collection in the SDR are in progress"
  ...
  def new_version_deposited_email
    ...
    @work_version = params[:work_version]
    @work = @work_version.work
    mail(to: @user.email, subject: "Updates to #{@work_version.title} have been deposited in the SDR")
  end
