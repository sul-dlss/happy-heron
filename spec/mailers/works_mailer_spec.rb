...
RSpec.describe WorksMailer do
  ...
  describe 'new_version_deposited_email' do
    ...
    it 'renders the headers' do
      expect(mail.subject).to eq 'Updates to twee retro man braid have been deposited in the SDR'
      expect(mail.to).to eq [user.email]
      expect(mail.from).to eq ['no-reply@sdr.stanford.edu']
    end
  ...
  describe 'new_version_reminder_email' do
    ...
    it 'renders the headers' do
      exp_subj = "Reminder: Updates to your deposit in the #{work.collection_name} " \
                 'collection in the SDR are in progress'
  ...
  describe 'decommission_owner_email' do
    ...
    it 'renders body' do
      expect(mail.body).to include "Your item \"#{work_version.title}\" has been removed"
    end
  end
  ...
  describe 'globus_endpoint_created' do
    ...
    it 'renders body' do
      ...
      expect(mail.body).to include "<a href=\"#{work_url(work)}\">Return to your item</a> at sdr.stanford.edu, " \
                                   'click on the link for the shared folder'
    end
  end
