...
RSpec.describe Dashboard::CollectionComponent, type: :component do
  ...
  context 'with 4 works' do
    ...
    it 'renders a table of works' do
      expect(rendered.css('.work-title a').pluck('href')).to include work_path
  ...
  context 'with a work that has a druid' do
    ...
    it 'renders a link to purl' do
      expect(rendered.css('a').pluck('href')).to include 'https://purl.stanford.edu/yq268qt4607'
    end

    it 'renders a copy link button with a unique aria-label' do
      expect(rendered.css('a').pluck('aria-label')).to include 'Copy persistent link ' \
                                                               "to #{WorkTitlePresenter.show(work.head)}"
    end
  end
end
