...
RSpec.describe 'Update an existing collection', :js do
  ...
  it 'updates the collection and adds to the event description history' do
    ...
    within '#events' do
      ...
      expect(page).to have_no_content 'collection name modified, description modified'
    end

    click_link_or_button 'Collection details'
    ...
    expect(page).to have_no_content(original_name)
    ...
    within '#events' do
      ...
      expect(page).to have_no_content 'contact email modified'
      expect(page).to have_no_content 'related links modified'
    end
