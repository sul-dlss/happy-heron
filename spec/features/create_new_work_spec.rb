  ...
  context 'when a user has previously accepted the terms of agreement less than 1 year ago' do
    ...
    context 'when successful deposit' do
      context 'with a user-supplied citation' do
        ...
        it 'deposits and renders work show page' do
          ...
          expect(page).to have_no_css('input#subtype_other') # rubocop:disable Capybara/SpecificMatcher
          find('label', text: 'Other').click
          expect(page).to have_css('input#subtype_other') # rubocop:disable Capybara/SpecificMatcher
          find('label', text: 'Sound').click

          check 'Podcast'
          expect(page).to have_no_content('Poetry reading')
          ...
          expect(page).to have_no_content "What's changing?"
          ...
          expect(page).to have_no_css("[aria-label='Delete #{WorkVersion.last.title}']")
          ...
          # NOTE: affiliations disabled for now
      ...
      context 'with an auto generated citation' do
        it 'deposits and renders work show page' do
          ...
          click_link_or_button 'See more options'
          expect(page).to have_no_css('#subtype_Data')
          ...
          expect(page).to have_content "Keller, M. (#{Time.zone.today.year}). My Title. Version 1. " \
                                       ...
                                       "Available at #{WorkVersion::LINK_TEXT}/version/1. " \
    ...
    context 'when unsuccessful' do
      ...
      it 'does not submit' do
        ...
        expect(page).to have_no_content('My Title')
        expect(page).to have_content('Deposit your content')
      end
    end
  end
