...
RSpec.describe 'Reserve a PURL for a work in a deposited collection', :js do
  ...
  context 'when depositor cannot select DOI' do
    ...
    it 'deposits a placeholder work' do
      ...
      click_link_or_button 'Reserve a PURL'
      fill_in 'Enter a title for this deposit', with: title
      expect(page).to have_no_content 'Do you want a DOI to be assigned to your deposit?'
      click_link_or_button 'Submit'
  ...
  describe 'setting the type for a reserved purl' do
    ...
    it 'from the dashboard' do
      ...
      within_table collection_version.name do
        click_link_or_button "Choose Type and Edit #{work_version.title}"
      end
