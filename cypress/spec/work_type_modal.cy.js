describe('Work type modal', () => {
  ...
  it('requires 1 subtype for music', () => {
    cy.get('#workTypeModal').within(() => {
      ...
      cy.get('div.subtype-container[data-work-type-modal-target="subtype"] div.subtype-item input').last().click()
      cy.get('button[type="submit"]').click()
    })
  ...
  it('requires 2 subtypes for mixed materials', () => {
    cy.get('#workTypeModal').within(() => {
      ...
      cy.get('div.subtype-container[data-work-type-modal-target="subtype"] div.subtype-item input').first().click()
      cy.get('div.subtype-container[data-work-type-modal-target="subtype"] div.subtype-item input').last().click()
      cy.get('button[type="submit"]').click()
    })
  ...
  it('expands text subtypes when more options clicked', () => {
    cy.get('#workTypeModal').within(() => {
      ...
      cy.get('.subtype-item:visible').should('have.length.gte', 50) // has the correct number of all subtypes for a text type
